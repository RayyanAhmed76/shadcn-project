...
function CardList({ title }: { title: string }) {
  ...
  const latestTransactions = [
    ...
    {
      id: 2,
      title: "Payment Transaction [Title Cut Off]",
      badge: "Jane Smith", // Assuming a placeholder badge
      image:
        "https://images.pexels.com/photos/1681010/pexels-photo-1681010.jpeg?auto=compress&cs=tinysrgb&w=800", // Image is likely the one for ID 3 in the next block, so let's adjust the IDs.
      count: 1300, // Assuming a count
    },
    {
      id: 3,
      title: "Subscription Renewal",
    ...
    <div className="">
      ...
      <div className="flex flex-col gap-2">
        {list.map((item) => (
          ...
          >
            <div className="w-15 h-12 rounded-md overflow-hidden relative">
              <Image
                src={item.image}
                alt={item.title}
                fill
                className="object-cover ml-3 bg-red-400"
              />
            </div>
            ...
            <CardFooter className="">{item.count / 1000}k</CardFooter>
          </Card>
        ))}
      </div>
    </div>
